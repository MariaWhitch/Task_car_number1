            ...
            if (answer.equals("1")) {
                ...
                if (number.matches("[АВЕКМНОРСТУХавекмнорстухABEKMHOPCTYXabekmhopctyx]\\s?\\d{3}\\s?[АВЕКМНОРСТУХавекмнорстухABEKMHOPCTYXabekmhopctyx]{2}(\\s?|\\s?([Rr][Uu][Ss])?|([Rr][Uu][Ss])?\\s?|\\s([Rr][Uu][Ss])\\s)\\d{2,3}")) {
                    ...
                    if (codeGBDD.length() == 3) {
                        ...
                        System.out.println("Такого кода ГБДД не существует. Пожалуйста уточните правильность номера и введите его заново.\n");
                    }
                    ...
                    System.out.println("Введённый номер: " + number + " - некорректный! Введены недопустимые буквы");
